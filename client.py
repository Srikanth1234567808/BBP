...
# List of tickers (for illustration, I'm using just a few here; you should have your list of 650 tickers)
...
# Define the Indian time zone (IST)
ist = pytz.timezone("Asia/Kolkata")

# Get the current date in YYYY-MM-DD format to use as the folder name
current_date = datetime.now().strftime("%Y-%m-%d")

# Create a folder named with the current date
folder_name = f"minute_data_{current_date}"
if not os.path.exists(folder_name):
    os.makedirs(folder_name)

# Batch size for downloading tickers
batch_size = 50  # Adjust as necessary
max_retries = 5  # Max retries for failed downloads
retry_delay = 5  # Delay between retries in seconds

def download_data_for_ticker(ticker):
    """Helper function to download and save data for a single ticker with retry logic."""
    retries = 0
    while retries < max_retries:
        try:
            ...
            # Convert the index (timestamp) to IST (Indian Standard Time)
            if data.index.tzinfo is None:  # If no timezone information exists
                data.index = data.index.tz_localize('UTC')
            data.index = data.index.tz_convert(ist)
            
            # Save the data to a CSV file
            file_path = os.path.join(folder_name, f"{ticker}_minute_data.csv")
            ...
            retries += 1
            print(f"Error downloading {ticker}: {e}. Retrying... ({retries}/{max_retries})")
            time.sleep(retry_delay + random.randint(0, 5))  # Add randomness to delay
...
for i in range(0, len(tickers), batch_size):
    ...
    print(f"\nProcessing batch {i // batch_size + 1} of tickers: {batch}")
    
    for ticker in batch:
        success = download_data_for_ticker(ticker)
        if not success:
            print(f"Skipping {ticker} due to repeated failures.")
    
    # Optionally, add a delay between batches to avoid hitting rate limits
    time.sleep(random.randint(5, 15))  # Random delay between batches

print(f"\nAll available data has been saved to the '{folder_name}' folder.")